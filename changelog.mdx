...
<Update label="August 29, 2026" description="Remote MCP carries Pick of the Day">
  [`POST /api/v1/mcp`](/api-reference/endpoint/remote-mcp) adds two read-only tools: `get_pick_of_the_day` dispatches to [`GET /api/v1/pick-of-the-day`](/api-reference/endpoint/get-pick-of-the-day) and `get_pick_of_the_day_archive` to [`GET /api/v1/pick-of-the-day/archive`](/api-reference/endpoint/get-pick-of-the-day-archive). Both take no arguments. `tools/list` now returns 33 tools.

  A tool error from any remote tool now carries the V1 `error.code`, `error.reason`, and `error.retry_at` when the handler set them. Before release, `get_pick_of_the_day` therefore reads `404 Not Found (not_found/pick_not_released): ... Not available before <retry_at>.` so an agent can schedule the next call instead of polling.

  The stdio package `@0xinsider/mcp` 1.2.0 serves the same 33 tools. Additive: existing tools and REST routes are unchanged.
</Update>

<Update label="August 29, 2026" description="Pick of the Day 404 retry_at now accounts for the live selector rescan">
  [`GET /api/v1/pick-of-the-day`](/api-reference/endpoint/get-pick-of-the-day) changes how `error.retry_at` and `Retry-After` are chosen on `404 pick_not_released`.

  Before this change the value was the earliest scheduled slot's `release_at`. The selector rescans about every 15 minutes and can add a slot that releases earlier. A client that honored `retry_at` slept past that pick. On August 29 the value read 16:15 UTC while a slot added at 11:15 UTC released at 12:30 UTC.

  The value is now the earliest of the next scheduled release and the next selector attempt, while the selector still scans the day. Expect more `404` responses on a busy day, about 15 minutes apart, until the first pick releases. The response shape is unchanged. A client that schedules from `retry_at` needs no change.
</Update>

<Update label="August 19, 2026" description="Pick of the Day announces same-day picks that have not dropped yet">
  ...
  Each slot carries exactly three fields - `pick_rank`, `release_at`, and `kickoff`. No matchup, category, side, price, or holder data exists on the shape before release, so the schedule can be shown without revealing a pick early.

  Use it to plan your next read: request once at the earliest `release_at` instead of re-polling. The field is present only while at least one unreleased slot remains, so its absence means the day is fully released.

  Additive and optional: existing clients are unaffected.
</Update>
