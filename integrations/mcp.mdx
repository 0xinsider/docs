...
description: "Run @0xinsider/mcp over stdio and give an AI client the 31 read-only tools."
---
...
<Note>
  A hosted agent that cannot run a local process can use [Remote MCP](/api-reference/endpoint/remote-mcp) at `POST /api/v1/mcp` instead. Both transports expose the same 31 tools and share the same rate limits.
</Note>
...
The server exposes 31 read-only tools from the same catalog as Remote MCP. It cannot place trades or write data.
